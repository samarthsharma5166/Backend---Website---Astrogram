                                    ...
                                    <div class="overflow-hidden ">
                                        <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                                            ...
                                            <tbody class="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                                                ...
                                                @foreach($data as $row)
                                                <tr>
                                                    <td width="10%" class="table-td">{{ $i++ }}</td>
                                                    <td width="10%" class="table-td">@if($row->img) <img src="{{ Asset('upload/category/'.$row->img) }}" style="height: 40px;"> @endif</td>
                                                    <td width="20%" class="table-td">{{ $row->name }}</td>
                                                    <td width="10%" class="table-td">

                                                        <a href="javascript:void(0)" onclick="showSweetAlert('category_status?id={{ $row->id }}','{{ __('admin.are_you_sure') }}','{{ __('admin.delete_confirm') }}')">
                                                    ...
                                                    <td width="50%" class="table-td" style="text-align: right;">
                                                        ...
                                                        <a href="javascript:void(0);" onclick="showSweetAlert('category_delete?id={{ $row->id }}')" class="btn inline-flex h-8 w-6 items-center justify-center btn-danger rounded-full toolTip onTop" data-tippy-content="{{ __('admin.delete') }}">
                                                            <span class="flex items-center">
