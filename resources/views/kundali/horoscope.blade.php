...
@section('title') {{ __('front.generate_horoscope') }} @endsection
...
<div class="container mx-auto px-4 py-12">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="text-center mb-10">
            <h1 class="text-3xl font-extrabold text-gray-900 mb-2">{{ __('front.generate_horoscope') }}</h1>
        ...
        <div class="mb-8">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex items-center justify-between">
                <div>
                    ...
                    <div class="text-2xl font-bold text-primary">{{ $setting->currency }}{{ number_format($setting->horoscope_cost, 2) }}</div>
                </div>
            ...
            <div class="mt-4 bg-red-50 border border-red-200 rounded-xl p-4 flex items-start space-x-3">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-red-600 text-sm mt-1">{{ __('front.need_more_balance_kundali', ['amount' => $setting->currency . number_format($setting->horoscope_cost - auth()->user()->wallet, 2)]) }}</p>
        ...
        <div id="kundaliFormContainer" class="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100">
            <form id="kundaliAjaxForm" action="{{ url('horoscope') }}" method="POST" class="p-8 space-y-6">
                @csrf
                ...
                <button type="submit" id="submitBtn"
                    ...
                    @if(!$haveBalance) disabled @endif>
                    <span id="btnText">{{ __('front.generate_horoscope_now') }}</span>
                ...
                <p class="text-center text-xs text-gray-400 mt-4">
                    {{ __('front.click_generate_deduct', ['amount' => $setting->currency . number_format($setting->horoscope_cost, 2)]) }}
                </p>
            </form>
        </div>
        ...
        <div id="kundaliResult" class="hidden mt-12 bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
            <div class="px-8 py-6 bg-primary/5 border-b border-gray-100 flex items-center justify-between">
                <h3 class="text-xl font-bold text-gray-900">{{ __('front.detailed_horoscope') }}</h3>
