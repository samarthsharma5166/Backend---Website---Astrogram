...
@section('title') {{ __('front.generate_predication') }} @endsection
...
<div class="container mx-auto px-4 py-12">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="text-center mb-10">
            <h1 class="text-3xl font-extrabold text-gray-900 mb-2">{{ __('front.generate_predication') }}</h1>
        ...
        <div class="mb-8">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex items-center justify-between">
                <div>
                    ...
                    <div class="text-2xl font-bold text-primary">{{ $setting->currency }}{{ number_format($setting->predication_cost, 2) }}</div>
                </div>
            ...
            <div class="mt-4 bg-red-50 border border-red-200 rounded-xl p-4 flex items-start space-x-3">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-red-600 text-sm mt-1">{{ __('front.need_more_balance_kundali', ['amount' => $setting->currency . number_format($setting->predication_cost - auth()->user()->wallet, 2)]) }}</p>
        ...
        <div id="kundaliFormContainer" class="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100">
            <form id="kundaliAjaxForm" action="{{ url('predication') }}" method="POST" class="p-8 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <label class="cursor-pointer">
                            <input type="radio" name="gender" value="Male" {{ (isset($userInfo['gender']) && $userInfo['gender'] == 'Male') ? 'checked' : (!isset($userInfo['gender']) ? 'checked' : '') }} class="peer hidden" @if(!$haveBalance) disabled @endif>
                            <div class="flex items-center justify-center p-3 border rounded-xl transition-all peer-checked:border-primary peer-checked:bg-primary/5 peer-checked:text-primary hover:bg-gray-50 text-xs font-semibold text-gray-700 @if(!$haveBalance) opacity-50 cursor-not-allowed @endif">
                                {{ __('front.male') }}
                            </div>
                        </label>
                        <label class="cursor-pointer">
                            <input type="radio" name="gender" value="Female" {{ (isset($userInfo['gender']) && $userInfo['gender'] == 'Female') ? 'checked' : '' }} class="peer hidden" @if(!$haveBalance) disabled @endif>
                            <div class="flex items-center justify-center p-3 border rounded-xl transition-all peer-checked:border-primary peer-checked:bg-primary/5 peer-checked:text-primary hover:bg-gray-50 text-xs font-semibold text-gray-700 @if(!$haveBalance) opacity-50 cursor-not-allowed @endif">
                                {{ __('front.female') }}
                            </div>
                        </label>
                        <label class="cursor-pointer">
                            <input type="radio" name="gender" value="Other" {{ (isset($userInfo['gender']) && $userInfo['gender'] == 'Other') ? 'checked' : '' }} class="peer hidden" @if(!$haveBalance) disabled @endif>
                            <div class="flex items-center justify-center p-3 border rounded-xl transition-all peer-checked:border-primary peer-checked:bg-primary/5 peer-checked:text-primary hover:bg-gray-50 text-xs font-semibold text-gray-700 @if(!$haveBalance) opacity-50 cursor-not-allowed @endif">
                                {{ __('front.other') }}
                            </div>
                        </label>
                ...
                <!-- Category Selection -->
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-3">{{ __('front.category') }}</label>
                    <div class="flex flex-wrap gap-2">
                        @foreach(['Financial', 'Career', 'Love', 'Health', 'General Life'] as $cat)
                        <label class="cursor-pointer">
                            <input type="radio" name="category" value="{{ $cat }}" {{ $loop->first ? 'checked' : '' }} class="peer hidden" @if(!$haveBalance) disabled @endif>
                            <div class="px-4 py-2 border rounded-full transition-all peer-checked:border-primary peer-checked:bg-primary/5 peer-checked:text-primary hover:bg-gray-50 text-xs font-semibold text-gray-700 @if(!$haveBalance) opacity-50 cursor-not-allowed @endif">
                                {{ $cat }}
                            </div>
                        </label>
                        @endforeach
                ...
                <!-- Timeline Selection -->
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-3">{{ __('front.timeline') }}</label>
                    <div class="flex flex-wrap gap-2">
                        @foreach(['This month', 'Coming 6 month', 'This complete year', 'Next 2 year'] as $time)
                        <label class="cursor-pointer">
                            <input type="radio" name="timeline" value="{{ $time }}" {{ $loop->first ? 'checked' : '' }} class="peer hidden" @if(!$haveBalance) disabled @endif>
                            <div class="px-4 py-2 border rounded-full transition-all peer-checked:border-primary peer-checked:bg-primary/5 peer-checked:text-primary hover:bg-gray-50 text-xs font-semibold text-gray-700 @if(!$haveBalance) opacity-50 cursor-not-allowed @endif">
                                {{ $time }}
                            </div>
                        </label>
                ...
                <button type="submit" id="submitBtn"
                    ...
                    @if(!$haveBalance) disabled @endif>
                    <span id="btnText">{{ __('front.generate_predication_now') }}</span>
                ...
                <p class="text-center text-xs text-gray-400 mt-4">
                    {{ __('front.click_generate_deduct', ['amount' => $setting->currency . number_format($setting->predication_cost, 2)]) }}
                </p>
            </form>
        </div>
        ...
        <div id="kundaliResult" class="hidden mt-12 bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
            <div class="px-8 py-6 bg-primary/5 border-b border-gray-100 flex items-center justify-between">
                <h3 class="text-xl font-bold text-gray-900">{{ __('front.detailed_predication') }}</h3>
